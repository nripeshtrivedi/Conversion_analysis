...
fitr <- kmeans(mydata, 3)
fitr1 <- kmeans(mydata_1, 2)
par(mar = rep(2, 4))
plot(mydata, col=fit$cluster)
set.seed(123)
k.max <- 15 # Maximal number of clusters
wss <- sapply(1:k.max, 
              function(k){kmeans(mydata_1, k, nstart=10 )$tot.withinss})
...
for(q in 1:length(list_final))
{
  ...
  clusts <- sort(unique(fitr$cluster))
  coeffs <- rep(-2,length(clusts))
  for(i in 1:length(fitr$cluster)){
    cdist <- rep(0,length(clusts))
    for(j in clusts){
      cdist[j] <- dist(rbind(fitr$centers[j,],mydata[i,]))
    }
    a <- cdist[fitr$cluster[i]]
    b <- min(cdist[clusts != fitr$cluster[i]])
